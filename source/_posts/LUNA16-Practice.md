...
# 预处理：提取肺部组织
数据预处理部分我感觉是涉及到了很多的医学知识。通过HU阈值、面积阈值、体积、slice的位置分布来判断提取出ROI。整个过程有点迭代的意思，还挺琐碎复杂的。
...
去掉边界。对于每一张slice，如果左上角$10 \times 10$的区域里，HU值都是一样的即都是空气或水，就用nan_mask将整张图的非中心区域去掉，否则就保留原值。然后再用用size = 1 pixel的高斯滤波器先进行滤波。最后，将$ HU >= -600 $的过滤，因为肺部组织的HU值是-500.
```python
        if len(np.unique(image[i, 0:bg_patch_size, 0:bg_patch_size])) == 1:
            current_bw = scipy.ndimage.filters.gaussian_filter(np.multiply.image[i].astype('float32'), nan_mask), 
                sigma, truncate=2.0) < intensity_th
...
下面两张图分别是处理前后用matplotlib画出来的图中我们可以看到这个步骤主要就是去掉外边框。
![处理前](binarized_NO1_slice60.png)
![处理后](load_NO1_slice60.png)
